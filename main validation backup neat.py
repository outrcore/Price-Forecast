...
stock_data, stats = preprocess_data(download_stock_data(ticker, '2023-05-01', '2024-05-01'))
...
def eval_genomes(genomes, config, train_data, test_data, close_mean, close_std):
    for genome_id, genome in genomes:
        ...
        for i in range(len(train_data) - 1):
            inputs = train_data.iloc[i][['Open', 'High', 'Low', 'Close', 'Volume', 'MA7']].values
        ...
        for i in range(len(test_data) - 1):
            inputs = test_data.iloc[i][['Open', 'High', 'Low', 'Close', 'Volume', 'MA7']].values
            output = net.activate(inputs)
            predicted_price = denormalize(output[0], close_mean, close_std)
...
stock_data, stats = preprocess_data(download_stock_data(ticker, '2023-05-01', '2024-05-1'))
close_mean, close_std = stats['Close']
